...
def resolve_image_path(path: str) -> str:
    p0 = path
    p = p0.replace('/Users/yyf/Mine_Space/18744/project/dataset/', '')
    p = p.lstrip('/')
    ...
    if "TLD/" in p:
        p2 = p[p.index("TLD/"):]
        cand = os.path.join(data_root, p2)
        if os.path.exists(cand):
            return cand

    if "/TLD/" in p0:
        p3 = p0.split("/TLD/", 1)[1]
        cand = os.path.join(data_root, p3)
        if os.path.exists(cand):
            return cand

    raise FileNotFoundError(f"Image not found. orig={path}")
...
def seq_eval_patched(cfg, loader, model, device, mode, epoch, work_dir, recoder):
    ...
    with torch.no_grad():
        for _, data in enumerate(tqdm(loader)):
            ...
            turn_sum_roi += int(data.get('car_num_roi', data.get('car_num', 0)))
            brake_sum_roi += int(data.get('car_num_roi', data.get('car_num', 0)))
            turn_sum_json += int(data.get('car_num_json', 0))
            brake_sum_json += int(data.get('car_num_json', 0))
    ...
    recoder.print_log(
        f'\tEpoch: {epoch} | ROI acc: Turn {roi_turn_acc*100:.2f}% Brake {roi_brake_acc*100:.2f}% '
...
import main
main.SLRProcessor.judge_save_eval = lambda self, epoch: (True, True)
