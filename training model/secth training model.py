...
def patched_normalize_and_crop(self, video, label, car_num):
    video_tensor = torch.from_numpy(video).permute(2, 0, 1).float()  # C,H,W
...
# set focal loss
processor.model.loss = FocalLoss(alpha=1.0, gamma=2.0, reduction="mean")
print("Current loss:", type(processor.model.loss), "alpha=1 gamma=2")
