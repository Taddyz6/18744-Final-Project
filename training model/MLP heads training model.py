...
def estimate_brake_counts_from_json(train_json_path: str):
    ...
    with open(train_json_path, 'r') as f:
        lst = json.load(f)
    for item in lst:
        labels = item.get('car_label', [])
        if not isinstance(labels, list):
            labels = [labels]
        for lab in labels:
            try:
                b = 0 if lab.get('brake_label') == 'car_BrakeOff' else 1
                if b == 0: c0 += 1
                else:      c1 += 1
            except:
                continue
...
bc = estimate_brake_counts_from_json(train_json)
if bc is None:
    train_json2 = os.path.join(project_path, 'TLD_YT_train.json')
    bc = estimate_brake_counts_from_json(train_json2)

if bc is None:
    brake_w = [1.0, 1.0]
    print("[Warn] Cannot find train json to estimate brake imbalance. Use brake_w=[1,1].")
else:
    c0, c1 = bc
    w0 = 1.0
    w1 = float(c0) / max(float(c1), 1.0)
    brake_w = [w0, w1]
    print(f"[Info] Brake counts from json: off(0)={c0}, on(1)={c1}, brake_w={brake_w}")
...
def patched_normalize_and_crop(self, video, label, car_num):
    ...
    for item in label:
        try:
            ...
            resized = resized.permute(1, 2, 0).contiguous()  # H,W,C
...
if not hasattr(main.SLRProcessor, "_orig_train__safe"):
    main.SLRProcessor._orig_train__safe = main.SLRProcessor.train

def _freeze_backbone_all(model):
    if hasattr(model, "conv2d"):
        for p in model.conv2d.parameters():
            p.requires_grad = False

def _unfreeze_layer4(model):
    if hasattr(model, "conv2d") and hasattr(model.conv2d, "layer4"):
        for p in model.conv2d.layer4.parameters():
            p.requires_grad = True

def train_warmup_then_unfreeze(self):
    ...
    _freeze_backbone_all(self.model)
    self.recoder.print_log(f'[Warmup] Freeze backbone. epochs={WARMUP_EPOCHS}')

    for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):

        if epoch == WARMUP_EPOCHS:
            if ONLY_LAYER4:
                _unfreeze_layer4(self.model)
                self.recoder.print_log('[FineTune] Unfreeze conv2d.layer4 only.')
            else:

                if hasattr(self.model, "conv2d"):
                    for p in self.model.conv2d.parameters():
                        p.requires_grad = True
                self.recoder.print_log('[FineTune] Unfreeze full backbone.')


            new_optim = dict(self.arg.optimizer_args)
            new_optim['base_lr'] = FT_LR
            self.optimizer = utils.Optimizer(self.model, new_optim)
            self.recoder.print_log(f'[FineTune] Rebuild optimizer AdamW, base_lr={FT_LR}.')
...
main.SLRProcessor.train = train_warmup_then_unfreeze
...
class TLD_resnet_mlphead(nn.Module):

    def __init__(self, loss_weights, head_hidden=256, head_dropout=0.3, brake_class_weight=None):
        super().__init__()
        ...
        # Per-task MLP heads (Option 1)
        h = int(head_hidden)
        p = float(head_dropout)

        self.fc_brake = nn.Sequential(
        ...
        if brake_class_weight is None:
            self.brake_loss = nn.CrossEntropyLoss()
        else:
            w = torch.tensor(brake_class_weight, dtype=torch.float32)
            self.brake_loss = nn.CrossEntropyLoss(weight=w)

    def forward(self, inputs_dict):
        x = inputs_dict['x'].permute(0, 3, 1, 2) 
        feat = self.conv2d(x)
        return {
            'turn_result':  self.fc_turn(feat),
            'brake_result': self.fc_brake(feat),
        }
    ...
    def get_loss(self, ret_dict, label):
        ...
        for k, weight in self.loss_weights.items():
            if k == 'turn':
                temp = float(weight) * self.turn_loss(ret_dict['turn_result'], labels_turn)
                loss += temp; loss_dict[k] = temp
            elif k == 'brake':
                temp = float(weight) * self.brake_loss(ret_dict['brake_result'], labels_brake)
                loss += temp; loss_dict[k] = temp
        return loss, loss_dict

light_network.TLD_resnet_mlphead = TLD_resnet_mlphead
...
print("Training schedule: epoch0 head-only (backbone frozen) -> epoch>=1 unfreeze layer4 + lr=3e-4")